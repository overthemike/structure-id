 ...
 */
export function generateStructureId(obj: Record<string, any>): string {
   ...
   */
  function getObjectSignature(obj: object, path: string[]): string {
    ...
    if (type === 'object') {
      ...
      return `${path.join('.')}.[${(obj as any[]).length}]`
    }
  }
 ...
 */
export function getStructureInfo(obj: Record<string, any>): {
  id: string
  levels: number
} {
